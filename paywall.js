...
var Paywall = (function () {
  ...
  function buildOverlay(opts, student) {
    ...
    var waText = 'PAYMENT / ACCESS REQUEST — TIH Learning Hub\n'
      ...
      + 'Item ID: ' + opts.itemId + '\n'
      + 'I have paid ' + P.currencyNote + ' via Mobile Money to ' + P.momoNumber + ' (' + P.momoName + '). '
    ...
    o.innerHTML =
      ...
      + '<p style="font-size:.92rem;color:#374151;margin:0 0 1rem;">Access to this course costs <b>US$' + esc(P.amountUSD) + '</b>. Follow these steps to unlock it:</p>'
      + '<ol style="margin:0 0 1rem 1.1rem;padding:0;font-size:.9rem;color:#374151;line-height:1.7;">'
      + '<li><b>Pay ' + esc(P.currencyNote) + '</b> by Mobile Money to:<br><span style="display:inline-block;background:#eef2ff;border:1px dashed #1E3A8A;border-radius:8px;padding:.5rem .8rem;margin-top:.35rem;font-weight:700;color:#1E3A8A;">' + esc(P.momoNumber) + ' &middot; ' + esc(P.momoName) + '</span></li>'
