...
(function () {
  ...
  function buildFlat(fromSections) {
    ...
    (fromSections || []).forEach(function (sec) {
      (sec.questions || []).forEach(function (qq) {
        var sh = shuffleQ(qq);
        flat.push({ section: sec.name, q: qq.q, opts: sh.opts, correct: sh.correct, exp: qq.exp, html: !!qq.html });
      });
    });
  }
  ...
  function renderQ() {
    ...
    var stem;
    if (qq.html) {
      stem = '<div class="q-stem q-html">' + qq.q + '</div>';
    } else {
      stem = '<div class="q-stem">' + esc(qq.q) + '</div>';
    }
    var h = stem;
  ...
  function finishTest() {
    ...
    flat.forEach(function (qq, i) {
      ...
      rev += '<strong>Q' + (i + 1) + '</strong> ' + (ok ? '✓' : '✗') + ' ' + esc(qq.q.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 120)) + '<br>';
      if (qq.opts) {
  ...
  function renderExtras(container) {
    ...
    (T.writing || []).forEach(function (w) {
      ...
      if (w.chartHtml) h += w.chartHtml;
      h += '<div class="p-q">' + esc(w.prompt) + '</div>';
      if (w.checklist) {
